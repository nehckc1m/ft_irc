...
int Server::check_authentication(Client &client, std::string cmd) {
    ...
    if (client.isAuthenticated()) {
        if ((client.getNickname().empty() && cmd != "NICK" && cmd != "USER") ||
            (client.getUsername().empty() && cmd != "USER" && cmd != "NICK")) {
            sendMessage(client.getFd(), "ERROR :You must register first (NICK and USER)\r\n");
            return 0;
        }
    }
    return 1;
